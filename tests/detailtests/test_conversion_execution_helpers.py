...
def test_convert_one_impl_uses_logged_sample_svg_for_manual_review_plan_b_status(tmp_path: Path) -> None:
    folder = tmp_path / "images"
    svg_out = tmp_path / "svg"
    diff_out = tmp_path / "diff"
    reports = tmp_path / "reports"
    ...
    for path in (folder, svg_out, diff_out, reports, samples):
        path.mkdir(parents=True, exist_ok=True)
    filename = "AC0VR2_AB_M.jpg"
    (folder / filename).write_bytes(b"fake")
    sample_svg = samples / "AC0VR2_AB_M.svg"
    sample_svg.write_text("<svg><rect id='exact-ac0vr2-ab'/></svg>", encoding="utf-8")
    (svg_out / "AC0VR2_AB_M.svg").write_text("<svg><rect id='bad-generated'/></svg>", encoding="utf-8")

    def _run(*_args, **_kwargs):
        return ("AC0VR2", "desc", {"mode": "manual_review"}, 1, 1.0)
    ...
    assert failed is False
    assert row is not None
    assert (svg_out / "AC0VR2_AB_M.svg").read_text(encoding="utf-8") == sample_svg.read_text(encoding="utf-8")

...
def test_make_ac08_badge_params_applies_rf_text_mode_for_ac0862() -> None:
    calls: list[str] = []
    ...
    assert calls == ["default_ac0812", "fit_ac0812", "finalize:AC0862", "enforce_left"]
    assert result["text_mode"] == "rf"
    assert result["label"] == "rF"
    assert result["draw_text"] is True
    assert result["enforced"] is True
...
def test_make_ac08_badge_params_uses_common_right_arm_finalizer_for_ac0864() -> None:
    calls: list[str] = []

    def _default_ac0814(_w: int, _h: int) -> dict:
        calls.append("default_ac0814")
        return {"seed": "ac0814", "stroke_gray": 150, "r": 8.0}

    def _fit_ac0814(_img, params: dict) -> dict:
        calls.append("fit_ac0814")
        patched = dict(params)
        patched["fit"] = True
        return patched

    def _finalize(name: str, params: dict) -> dict:
        calls.append(f"finalize:{name}:{params['connector_direction']}")
        patched = dict(params)
        patched["finalized"] = name
        return patched

    def _enforce_right(params: dict, _w: int, _h: int) -> dict:
        calls.append(f"enforce_right:{params['connector_direction']}")
        patched = dict(params)
        patched["enforced_right"] = True
        return patched
    ...
    result = helpers.makeAc08BadgeParamsImpl(
        ...
        default_ac0812_params_fn=lambda *_: _stub_params("ac0812"),
        default_ac0813_params_fn=lambda *_: _stub_params("ac0813"),
        default_ac0814_params_fn=_default_ac0814,
        default_ac0881_params_fn=lambda *_: _stub_params("ac0881"),
        default_ac0882_params_fn=lambda *_: _stub_params("ac0882"),
        fit_ac0870_params_from_image_fn=lambda img, defaults: defaults,
        fit_semantic_badge_from_image_fn=lambda img, defaults: defaults,
        ...
        fit_ac0812_params_from_image_fn=lambda img, defaults: defaults,
        fit_ac0813_params_from_image_fn=lambda img, defaults: defaults,
        fit_ac0814_params_from_image_fn=_fit_ac0814,
        apply_co2_label_fn=lambda p: p,
        apply_voc_label_fn=lambda p: p,
        tune_ac0831_co2_badge_fn=lambda p: p,
        tune_ac0832_co2_badge_fn=lambda p: p,
        ...
        finalize_ac08_style_fn=_finalize,
        enforce_left_arm_badge_geometry_fn=lambda p, _w, _h: p,
        enforce_right_arm_badge_geometry_fn=_enforce_right,
    )

    assert calls == ["default_ac0814", "fit_ac0814", "finalize:AC0864:right", "enforce_right:right"]
    assert result["connector_direction"] == "right"
    assert result["text_mode"] == "rf"
    assert result["label"] == "rF"

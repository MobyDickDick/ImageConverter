...
def test_build_prepare_run_iteration_pipeline_locals_for_run_call_kwargs_impl_returns_run_call_mapping() -> None:
    marker = object()
    ...
    assert result["run_seed"] == 11
    assert result["pass_seed_offset"] == 5
    assert result["img_path"] == "images/AC0800_L.jpg"
    assert result["iteration_run_preparation_helpers"] is marker
    assert "prepare_iteration_input_runtime_for_run_fn" not in result

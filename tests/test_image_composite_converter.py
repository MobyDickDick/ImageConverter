...
def test_generate_badge_svg_renders_square_badge_with_explicit_canvas_inset() -> None:
    """Square-head badges can carry measured full-canvas insets without catalog IDs."""
    params = {
        "circle_enabled": True,
        "head_style": "square_badge",
        "head_fill": "#fc0200",
        ...
        "square_badge_x": 1.0,
        "square_badge_y": 1.25,
        "square_badge_side": 26.0,
        "square_badge_height": 26.5,
        "cx": 14.0,
        "cy": 14.0,
        "r": 13.0,
        "fill_gray": 217,
    ...
    assert '<rect x="1.0000" y="1.2500" width="26.0000" height="26.5000" fill="#fc0200" stroke="#fc0200"' in svg
    assert "<circle" not in svg
...
def test_validate_badge_by_elements_raises_timeout_error_when_budget_exceeded() -> None:
    ...
    class _FakeTime:
        def __init__(self) -> None:
            self._values = iter((0.0, 0.02, 0.02, 0.02))

        def monotonic(self) -> float:
            return next(self._values)

    img = image_composite_converter.np.zeros((4, 4, 3), dtype=image_composite_converter.np.uint8)
    params = {"validation_time_budget_sec": 0.01}

    with pytest.raises(TimeoutError, match="validation_time_budget_exceeded"):
        validation_helpers.validateBadgeByElementsImpl(
            img,
            params,
            max_rounds=1,
...
def test_template_transfer_skips_dual_arrow_badge_target(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Dual-arrow badges (e.g. AC0023) must never be replaced by donor templates."""
    if image_composite_converter.np is None or image_composite_converter.cv2 is None:
        pytest.skip("numpy/cv2 not available in this environment")
    ...
    img = np.full((35, 20, 3), 240, dtype=np.uint8)
    target_filename = "AC0023.jpg"
    cv2.imwrite(str(folder / target_filename), img)
    ...
    (svg_dir / "AC0023.svg").write_text(
        '<svg width="20" height="35" xmlns="http://www.w3.org/2000/svg"></svg>',
        encoding="utf-8",
    )

    donor_params = Action.make_badge_params(50, 75, "AC0223")
    assert donor_params is not None
    donor_params["mode"] = "semantic_badge"
    (svg_dir / "AC0223_M_sia.svg").write_text(Action.generate_badge_svg(50, 75, donor_params), encoding="utf-8")
...
def test_read_svg_geometry_detects_co2_text_from_single_text_node(tmp_path: Path) -> None:
    ...
    svg_path.write_text(
        ...
        '<text x="12.5" y="16" fill="#6f6f6f">CO2</text>'
        "</svg>",
        encoding="utf-8",
    )
...
def test_read_svg_geometry_detects_split_co2_text_nodes(tmp_path: Path) -> None:
    svg_path = tmp_path / "AC0831_M.svg"
    svg_path.write_text(
        '<svg width="25px" height="45px" viewBox="0 0 25 45" xmlns="http://www.w3.org/2000/svg">'
        '<circle cx="12.5" cy="12.5" r="10" fill="#efefef" stroke="#9b9b9b" stroke-width="1"/>'
        '<rect x="12" y="22" width="1" height="23" fill="#9b9b9b"/>'
        '<text x="12.5" y="16" fill="#6f6f6f">CO</text>'
        '<text x="17" y="13" fill="#6f6f6f">2</text>'
        "</svg>",
        encoding="utf-8",
    )

def _module_call_edges_for_path(module_path: str | os.PathLike[str]) -> tuple[dict[str, int], list[dict[str, object]]]:
    ...
    chunk_dir = source_path.parent / "mainFiles" / "image_composite_converterFiles"
    if (
        source_path.name == "image_composite_converter.py"
        and "Auto-generated loader for split module chunks" in source
        and chunk_dir.exists()
    ):
        chunk_paths = sorted(chunk_dir.glob("image_composite_converter.chunk_*.py"))
        if chunk_paths:
            source = "".join(path.read_text(encoding="utf-8") for path in chunk_paths)

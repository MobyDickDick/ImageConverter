"""Ranking/reporting helper functions extracted from imageCompositeConverter."""
...
def writePixelDelta2RankingImpl(
    folder_path: str,
    svg_out_dir: str,
    reports_out_dir: str,
    *,
    ...
    for svg_name in sorted(f for f in os.listdir(svg_out_dir) if f.lower().endswith(".svg")):
        ...
        img_orig = cv2_module.imread(orig_path)
        if img_orig is None:
            continue
        ...
        rendered = render_svg_to_numpy_fn(svg_content, w, h)
        if rendered is None:
            continue

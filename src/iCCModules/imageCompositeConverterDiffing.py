...
def createDiffImageWithoutCv2Impl(input_path, svg_content, *, fitz_module):
    ...
    with fitz_module.open(str(input_path)) as original_doc, fitz_module.open("pdf", svg_content.encode("utf-8")) as svg_doc:
        ...
        if (svg_pix.width, svg_pix.height) != (original_pix.width, original_pix.height):
            svg_pix = fitz_module.Pixmap(svg_pix, original_pix.width, original_pix.height)
        ...
        diff_pix = fitz_module.Pixmap(fitz_module.csRGB, original_pix.width, original_pix.height, bytes(diff_samples), 0)

# Nächstes Arbeitspaket – DLG0021 Attofine-Gradient-Offset-Probes Run VR (2026-07-07)

Run VR rotiert nach dem GE9013_1M-Picofine-Warmfill-Schritt aus Run VQ zurück
...
um eine noch feinere attofeine Stufe ergänzt.

## 1) Umsetzung

- Die allgemeine Geometry-IR-Optimierung probt für vorhandene
  `stroke_gradient.stops` zusätzlich attofeine Offset-Deltas von `±0.0001953125`
  relativ zum aktuellen Stop-Offset.
- Die neue Stufe liegt zwischen dem femtofeinen Schritt `±0.000390625` und dem
...
`nur Sonderfall`-Signal. Run VR erweitert nicht die Bilddetektion, sondern den
...
- `PYTHONPATH=vendor/linux-py310/site-packages:. python -m pytest -q tests/detailtests/test_geometry_ir_optimizer_helpers.py::test_default_optimizer_refines_polygon_path_stroke_gradient_offsets_with_attofine_probe tests/detailtests/test_geometry_ir_optimizer_helpers.py::test_default_optimizer_refines_polygon_path_stroke_gradient_offsets_with_femtofine_probe tests/detailtests/test_geometry_ir_optimizer_helpers.py::test_default_optimizer_refines_polygon_path_stroke_gradient_offsets_with_picofine_probe` läuft grün mit `3 passed`.
...
Run VR schließt den dokumentierten DLG0021-Feinschritt ab. `PolygonPath`-
Stroke-Gradient-Offsets können nun katalogfrei attofeine Nachbarwerte bewerten;

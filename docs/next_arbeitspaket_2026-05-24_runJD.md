...
- Befehl:
  - `PYTHONPATH=. python3 tools/plan_b_synthetic_probe.py "Bildbeschreibung: Kreis mit horizontalem Strich in der Mitte." --variant AC0030_S --output-dir artifacts/converted_images/reports`
- Ergebnis:
  - `status=ok`, `variant=AC0030_S`, Exit `0`.
- Log:
  - `artifacts/converted_images/reports/AC0030_S_planb_synthetic_2026-05-24_runJD.log`

## 3) Nächstes CSV-Bild (AC0030_S)
- Befehl:
...
- Ergebnis:
  - Lauf endet mit Exit `0`; Plan-B-Sample-Vergleich für `AC0030_S` wurde genutzt (`err=217.940`, `baseline=227.067`).
- Log:

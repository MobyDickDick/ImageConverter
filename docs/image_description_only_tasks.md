...
- keine Runtime-Bild-ID-Vorkommen in `src/`; die frühere Legacy-Baseline ist
  entfernt.
...
- [x] **IDO-18 – Legacy-Baseline und Ratchet entfernen**
  ...
  - Umsetzung: Run SH löscht die Migrationsbaseline, entfernt `--baseline` und
    `--update` aus `tools/check_no_new_image_id_hardcoding.py` und macht die
    Prüfung zur absoluten Null-Vorkommen-Regel. CLI- und Unit-Tests sichern,
    dass synthetische Runtime-IDs ohne Allowlist fehlschlagen und der reale
    `src/`-Baum `0` Vorkommen meldet.

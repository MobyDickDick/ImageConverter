...
- [x] T6: Sämtliche aktuell blockierenden Langläufer-Tests identifizieren und priorisiert abbauen (Stand: Volltest-Isolation vom 2026-05-03).
  ...
  - [x] T6.1 (sehr hohe Priorität): `tests/test_image_composite_converter.py::test_ac08_semantic_anchor_variants_convert_without_failed_svg` reduzieren (historisch `377.98s`, abgeschlossen in Run PK mit `3.26s` Pytest-Laufzeit).
    ...
    - [x] T6.1.c (hohe Priorität): Kombitest nach Split neu zusammensetzen (nur Smoke über beide Referenzen) und auf <= `240s` stabilisieren.
      ...
      - 2026-06-14 (Run PK): Der Smoke-Test übernimmt `AC0811_L.jpg` aus dem heutigen Ablageort `artifacts/images_to_convert/nonconvertable/` in ein isoliertes temporäres Eingabeverzeichnis und läuft dadurch statt eines Fixture-Skips real über beide Referenzen. Der timeout-gesicherte Lauf endet mit Exit `0`, `1 passed in 3.26s` (`4.059s` Wandzeit) und bestätigt für `AC0811_L` sowie `AC0812_M` echte SVG-Ausgaben ohne `*_failed.svg`; Log: `artifacts/converted_images/reports/T6_1c_smoke_2026-06-14_runPK.log`. Damit sind T6.1.c und die übergeordnete T6.1-Laufzeitaufgabe abgeschlossen.
  - [x] T6.2 (sehr hohe Priorität): `tests/test_image_composite_converter.py::test_ac08_regression_suite_preserves_previously_good_variants[AC0837_L-semantic_ok]` reduzieren (historisch `198.28s`, abgeschlossen in Run PL mit `113.07s` Pytest-Laufzeit).
    ...
    - 2026-06-14 (Run PL): Der dokumentierte Skip wurde entfernt. Der AC0837-L-Fall läuft nun als isolierte Einvarianten-Konvertierung mit zwei Iterationen und einem auf 60 Sekunden begrenzten Elementvalidierungsbudget; SVG, fehlendes `*_failed.svg` und `status=semantic_ok` werden weiterhin explizit geprüft. Der timeout-gesicherte Lauf endet mit Exit `0`, `1 passed in 113.07s` (`113.869s` Wandzeit) und erfüllt damit das 120-Sekunden-Ziel; Log: `artifacts/converted_images/reports/T6_2_ac0837L_isolation_2026-06-14_runPL.log`.

  - [x] T6.3 (sehr hohe Priorität): `tests/test_image_composite_converter.py::test_make_badge_params_keeps_ac0838_m_circle_near_full_width_for_voc_layout` reduzieren (historisch `173.27s`, abgeschlossen in Run PM mit `60.14s` Pytest-Laufzeit).
    ...
    - 2026-06-14 (Run PM): Der Geometrie-Regressionstest begrenzt die Elementvalidierung auf eine vollständige Optimierungsrunde. Die drei bestehenden Radius-/Zentrums-Assertions bleiben unverändert. Der echte Heavy-Test endet unter einem 90-Sekunden-Guard mit Exit `0`, `1 passed in 60.14s` (`60.947s` Wandzeit) und erfüllt damit das Laufzeitziel; Log: `artifacts/converted_images/reports/T6_3_ac0838M_isolation_2026-06-14_runPM.log`.
  - [x] T6.4 (sehr hohe Priorität): `tests/test_image_composite_converter.py::test_ac08_regression_suite_preserves_previously_good_variants[AC0820_L-semantic_ok]` reduzieren (historisch `168.27s`, abgeschlossen in Run PN mit `55.67s` Pytest-Laufzeit).
    ...
    - 2026-06-14 (Run PN): Der AC0820-L-Regressionsfall führt weiterhin die echte semantische Pipeline aus, begrenzt aber nur für diese parametrisierte Variante die Elementvalidierung auf eine vollständige Runde. Der Test prüft zusätzlich explizit, dass das Validierungslog keinen `validation_time_budget_exceeded`-Marker enthält. Der Heavy-Test endet mit Exit `0`, `1 passed in 55.67s` (`56.511s` Wandzeit) und erfüllt das 120-Sekunden-Ziel; Log: `artifacts/converted_images/reports/T6_4_ac0820L_isolation_2026-06-14_runPN.log`.
  ...
  - [x] T6.8 (hohe Priorität): `tests/test_image_composite_converter.py::test_validate_badge_can_expand_ac0812_tiny_circle_radius` reduzieren (historisch `101.94s`, abgeschlossen in Run PR).
    - Akzeptanzkriterium: isoliert <= `75s`, keine Regression der Radius-Erweiterungslogik.
    - 2026-06-14 (Run PR): Der unveränderte fokussierte Radius-Test wurde auf der aktuellen Toolchain erneut timeout-gesichert ausgeführt. Die bestehende Zwei-Runden-Validierung erweitert den absichtlich auf `5.0` gesetzten Radius weiterhin und protokolliert das Radius-Bracketing. Der Lauf endet unter dem 75-Sekunden-Guard mit Exit `0`, `1 passed in 3.10s` (`4.184s` Wandzeit); eine weitere Reduktion des Testbudgets oder Produktionscodeänderung ist nicht erforderlich. Log: `artifacts/converted_images/reports/T6_8_ac0812_radius_2026-06-14_runPR.log`.
  - [x] T6.9 (mittel-hohe Priorität): `tests/test_image_composite_converter.py::test_validate_badge_by_elements_activates_ac08_adaptive_unlocks_on_stagnation` reduzieren (historisch `65.09s`, abgeschlossen in Run PS).
    - Akzeptanzkriterium: isoliert <= `45s`, Unlock-Verhalten bleibt testbar erhalten.
    - 2026-06-14 (Run PS): Der unveränderte fokussierte Stagnationstest wurde auf der aktuellen vendorten Python-3.10-Toolchain mit aktiviertem Heavy-Test-Pfad, einem äußeren 45-Sekunden-Guard und demselben pytest-Einzeltest-Timeout ausgeführt. Der Test durchläuft weiterhin vier Validierungsrunden und prüft, dass die nicht freigeschaltete AC0831-Familie keinen `adaptive_unlock_applied`-Marker erhält. Der Lauf endet mit Exit `0`, `1 passed in 4.27s` (auf ganze Sekunden gemessene Wandzeit `6s`); eine Verkürzung des fachlichen Testpfads oder Änderung am Produktionscode ist nicht erforderlich. Log: `artifacts/converted_images/reports/T6_9_adaptive_unlock_2026-06-14_runPS.log`.
  - [x] T6.10 (mittel-hohe Priorität): `tests/test_image_composite_converter.py::test_validate_badge_logs_extent_bracketing_for_line_elements` reduzieren (historisch `51.61s`, abgeschlossen in Run PT).
  ...
  - [x] T6.12 (hoch): `tests/test_image_composite_converter.py::test_ac08_regression_suite_preserves_previously_good_variants[AC0800_L-semantic_ok]` isolieren und Laufzeit dokumentiert reduzieren (abgeschlossen in Run PV).
    - Akzeptanzkriterium: isoliert <= `120s`, Status weiterhin `semantic_ok`.
    - 2026-06-15 (Run PV): Der unveränderte parametrisierte AC0800-L-Regressionsfall wurde mit aktiviertem Heavy-Test-Pfad, einem äußeren 120-Sekunden-Guard und demselben pytest-Einzeltest-Timeout isoliert ausgeführt. Der aktuelle semantische Pfad benötigt keine weitere Verkürzung: Der Lauf endet mit Exit `0`, `1 passed in 4.71s` (auf ganze Sekunden gemessene Wandzeit `6s`) und bestätigt weiterhin `status=semantic_ok`. Produktions- und Testcode bleiben unverändert. Log: `artifacts/converted_images/reports/T6_12_ac0800L_isolation_2026-06-15_runPV.log`.
  - [x] T6.13 (hoch): `tests/test_image_composite_converter.py::test_ac08_regression_suite_preserves_previously_good_variants[AC0800_M-semantic_ok]` isolieren und Laufzeit dokumentiert reduzieren (abgeschlossen in Run PW).
    - Akzeptanzkriterium: isoliert <= `120s`, Status weiterhin `semantic_ok`.
    - 2026-06-15 (Run PW): Der unveränderte parametrisierte AC0800-M-Regressionsfall wurde mit aktiviertem Heavy-Test-Pfad, einem äußeren 120-Sekunden-Guard und demselben pytest-Einzeltest-Timeout isoliert ausgeführt. Der aktuelle semantische Pfad benötigt keine weitere Verkürzung: Der Lauf endet mit Exit `0`, `1 passed in 3.03s` (auf ganze Sekunden gemessene Wandzeit `5s`) und bestätigt weiterhin `status=semantic_ok`. Produktions- und Testcode sowie reguläre Konvertierungsbudgets bleiben unverändert. Log: `artifacts/converted_images/reports/T6_13_ac0800M_isolation_2026-06-15_runPW.log`.
...
- [ ] **A3 Standard-Run pro Block**
  ...
  - **Session 2026-08-05 (Run ABI, B-AC05-01):** Nächster Standard-Run mit fixer Toolchain und äußerem Timeout erfolgreich ausgeführt: `PYTHONPATH=vendor/linux-py310/site-packages:. PYENV_VERSION=3.10.20 timeout 300 python -m src.iCCModules.imageCompositeConverterCli artifacts/images_to_convert --descriptions-path artifacts/images_to_convert/Finale_Wurzelformen_V3.xml --output-dir /tmp/ic-b-ac05-01-20260805 --start AC0501 --end AC0532 --deterministic-order 2>&1 | tee artifacts/converted_images/reports/B-AC05-01_standard_run_2026-08-05.log` endete mit Exit `0`. Die Kurzprüfung ergab `193` Konvertiert-Meldungen für `84` eindeutige Varianten aus allen `10` geplanten Basis-IDs (`AC0501`, `AC0502`, `AC0503`, `AC0504`, `AC0511`, `AC0512`, `AC0521`, `AC0522`, `AC0531`, `AC0532`), also keine lokale Datenlücke. Der Lauf enthält keinen Timeout, keinen Traceback, kein `conversion_failed` und keine `ERROR`-Meldung; auffällig bleiben `162` Qualitäts-`WARN`-Meldungen, vor allem konzentrierte Differenzbild-Fehlerpixel und manuelle Review-Elementfits für unklassifizierte `AC0503`/`AC0504`-SIA-Varianten, daher ist der Block als laufzeitstabiler Standard-Run mit Qualitätsfolgepunkten in A4 zu reviewen.
...
- **Fortschritt (Primäraufgabe A4):** Der Review für den bereits vorhandenen Standard-Run `B-AC08-02` wurde nachgetragen und direkt im Taskboard mit Log-Pfad, Ergebnis und Kurznotiz dokumentiert.
- **Ergebnis:** `artifacts/converted_images/reports/B-AC08-02_standard_run_2026-07-02.log` bleibt als `stabil mit Qualitätsfolgepunkten` bewertet: Exit `0`, Abschlussmarker, keine Datenlücke, kein Timeout, kein Traceback und kein `conversion_failed`.
- **Befund:** Die Kurzprüfung bestätigt `62` Konvertiert-Meldungen für alle `30` erwarteten Varianten aus `AC0841` bis `AC0850`; vier `AC0843_*`-Semantik-ERRORs wurden abgefangen, und `48` Qualitäts-WARNs bleiben als Review-/A5-Folgepunkte sichtbar.
- **Nächster sinnvoller Schritt:** Mit `B-AC08-03` per A3 fortfahren; A5 erst nach drei reviewten Blöcken bündeln, damit der Qualitäts-Checkpoint genügend Vergleichsbasis hat.
...
- **Nächster sinnvoller Schritt:** A4-Review für `B-AC08-02` ist mit Run UJ nachgetragen; als nächstes `B-AC08-03` per A3 ausführen und danach A5 erst nach dem dritten reviewten Block auslösen.
...
### Fortschritt vs. Blocker (Session 2026-06-11, Plan-B AC0733_1_M Run OY)

- **Fortschritt:** `AC0733_1_M.jpg` wurde real re-konvertiert; das für die große AC0733-Variante eingeführte beschreibungsgetriebene Geometry-IR überträgt Anschluss, roten Quadratgrundkörper und horizontal bleibenden P-Glyph ohne neue variantenspezifische Geometrie auf 20x35 Pixel.
- **Messwert:** `mean_delta2` sinkt von `17248.937500` auf `3555.331543` (`-79.39 %`), `normalized_mse` von `0.08842208` auf `0.01822546` und damit unter die Review-Grenze `0.04594568`.
- **Perception-Lerneffekt:** Der Geometry-IR-Familienpfad ist über große und mittlere Realvariante generalisiert; PF8 benötigt weiterhin allgemeine Rechteck-/Text-Seeds statt des bislang unvollständigen Linienhinweises.
- **Rotation:** `AC0733_1_M` wurde aus Triage und PF8-Linkage entfernt; `AC0254_2` füllt den fünften Platz auf und bleibt mit einer Linienerkennung ohne passenden Rechteck-/Rule-Seed zunächst `nur Sonderfall`.
- **Nächster sinnvoller Schritt:** Die reguläre Plan-B-Rotation mit `AC0722_1_L.jpg` fortsetzen.


### Fortschritt vs. Blocker (Session 2026-06-11, Plan-B AC0722_1_L Run OZ)

- **Fortschritt:** `AC0722_1_L.jpg` wurde real re-konvertiert; ein neues beschreibungsgetriebenes Geometry-IR trennt horizontalen Anschluss, roten Quadratgrundkörper und horizontal bleibenden T-Glyph ohne variantenspezifische Pixelkoordinaten.
- **Messwert:** `mean_delta2` sinkt von `14995.260742` auf `4721.479004` (`-68.51 %`), `normalized_mse` von `0.07686921` auf `0.02420340` und damit unter die Review-Grenze `0.04594568`.
...
### Fortschritt vs. Blocker (Session 2026-06-12, Plan-B AC0732_1_M Run PB)

- **Fortschritt:** `AC0732_1_M.jpg` wurde real re-konvertiert; ein neues beschreibungsgetriebenes Geometry-IR toleriert `gredreht` und trennt linken Anschluss, roten Quadratgrundkörper und horizontal bleibenden P-Glyph ohne variantenspezifische Pixelkoordinaten.
- **Messwert:** `mean_delta2` sinkt von `13642.634766` auf `3317.628662` (`-75.68 %`), `normalized_mse` von `0.06993533` auf `0.01700694` und damit unter die Review-Grenze `0.04594568`.
...
### Fortschritt vs. Blocker (Session 2026-07-03, GE9012_6M Microfine-Opacity-Probes Run UT)

- **Fortschritt:** Der aktive Plan-B-Kandidat `GE9012_6M` wurde mit einer weiteren katalogfreien Geometry-IR-Optimiererweiterung abgesichert: `ColorPatch`- und `RectBorder`-Elemente prüfen nun zusätzliche 0,003125er-Zwischenstufen für `fill_opacity` und `stroke_opacity`.
- **Sicherung:** Der fokussierte Geometry-IR-Optimierer-Testblock läuft grün; die absolute Runtime-ID-Nullprüfung meldet weiterhin `0 occurrences`. Der isolierte CLI-Lauf für `GE9012_6M` endet mit Exit `0` und bleibt metrisch stabil.
...
### Fortschritt vs. Blocker (Session 2026-07-08, DLG0021 Zeptofine-Gradient-Offset-Probes Run VW)

- **Fortschritt:** Der dokumentierte DLG0021-Gradient-Folgepunkt wurde als katalogfreie Geometry-IR-Optimiererweiterung umgesetzt: `PolygonPath`-Stroke-Gradient-Offsets prüfen nun zusätzlich zeptofeine ±0,009765625-Prozentpunkte.
- **Sicherung:** Der fokussierte Geometry-IR-Optimierer-Testblock läuft grün; die absolute Runtime-ID-Nullprüfung meldet weiterhin `0 occurrences`. Der isolierte CLI-Lauf für `DLG0021` endet mit Exit `0`.
